...
# ========= CLI =========
parser = argparse.ArgumentParser(description="Run VL anomaly detector with configurable modes.")
parser.add_argument(
    "--mode", "-m",
    choices=["v0", "v1", "v2", "v3"],
    default="v3",
    help=(
        "v0: text-only timestamp/value without description; "
        "v1: text-only timestamp/value with description; "
        "v2: text-only timestamp/value/moving_average/moving_std with description; "
        "v3: VL mode with timestamp/value/moving_average/moving_std and raw/mean/std images."
    ),
)
...
IS_TEXT_ONLY: bool = IMG_MODE in {"v0", "v1", "v2"}

# Resolve paths from arguments
csv_dir = args.csv_dir.resolve()
img_dir = args.img_dir.resolve()

# Toujours charger le modèle VL, même pour les variantes text-only
...
def _named_linear_modules(m):
    return {
        ...
        if hasattr(mod, "weight") and getattr(mod, "weight") is not None
    }
...
IMAGE_MODE_DESCRIPTIONS: Dict[str, str] = {
    ...
    "v1": "",
    "v2": "",
    "v3": " and the following plot images: raw values, moving average, and moving standard deviation",
}
...
PROMPT_TEMPLATE_V0 = (
    "Given the time series below{images_note}, determine whether there is an anomalous interval.\n"
    "If the series is entirely normal, return the empty JSON template. Otherwise, detect the anomalous interval.\n\n"
    "Return ONLY a JSON object formatted exactly as follows, with no extra keys or text:\n\n"
    "Empty (no anomaly):\n"
    "{{\n  \"anomalies\": []\n}}\n\n"
    "Non-empty:\n"
    ...
    "* Do NOT use keys other than \"anomalies\", \"start\", and \"end\".\n"
    "* Do NOT invent anomalies when none exist.\n\n"
    "Time series (len: {length}):\n"
    "{header}\n"
    "{series}"
...
for fname in tqdm(sorted(os.listdir(csv_dir))):
    ...
    prompt_template = PROMPT_TEMPLATE_V0 if IMG_MODE == "v0" else PROMPT_TEMPLATE_BASE

    prompt_txt = prompt_template.format(
        length=len(df),
