...
def print_error_attribution_summary(stats: Dict[str, Dict[str, int]], total_records: int):
    ...
    for p_type in sorted(stats.keys()):
        ...
        # Shares among false citations for that pattern type
        if f > 0:
            p_fc = 100 * fc / f
            p_ti = 100 * ti / f
            p_oth = 100 * oth / f
...
def build_context_window(
    ...
    """
    Retourne un extrait de la série autour de ts (± window).
    Si ts est hors bornes, on "clamp" vers le ts existant le plus proche.
    """
    if not data_map:
    ...
    # Clamp vers un timestamp existant proche
    if ts <= min_ts:
        center = min_ts
    elif ts >= max_ts:
        center = max_ts
...
def format_context_for_console(ctx: Dict[str, Any]) -> str:
    """
    Formatte un contexte (rows) en petit tableau texte.
    """
    rows = ctx.get("rows", [])
    if not rows:
        return "  (aucune donnée de contexte)\n"

    # Collecter toutes les features vues
    feats = set()
    for r in rows:
        for k in r.keys():
    ...
    out = []
    out.append(
        f"  Contexte (requested_ts={ctx.get('requested_ts')}, center_ts={ctx.get('center_ts')}, bounds={ctx.get('bounds')}):"
    )
    out.append("  " + "-" * 78)
...
def verify_citations(
    ...
    against the provided data_map.
    Ajoute failed_examples: liste d'erreurs avec contexte autour du timestamp.

    IMPORTANT (mise à jour):
      - Toutes les valeurs sont des entiers.
      - Une citation est correcte si et seulement si la valeur est exactement égale.
      - Nouvelles métriques exclusives:
          * feature_confusion: valeur correcte pour une autre feature au même timestamp (match exact)
          * time_imprecision: valeur correcte pour la même feature à ts±2 (match exact)
        Priorité: feature_confusion > time_imprecision > other
    """
    ...
    # --- New exclusive error attribution counters ---
    feature_confusion_errors = 0
    time_imprecision_errors = 0
    other_errors = 0
    ...
    def add_fail_example(example: Dict[str, Any]):
        # On garde quelques exemples, pas tout, pour éviter un JSON énorme
    ...
    def _feature_confusion_same_ts(ts_: int, cited_feature_: str, cited_val_: int) -> Optional[str]:
        """
        Si cited_val_ correspond exactement à une AUTRE feature au même ts_,
        retourne le nom de la feature matchée, sinon None.
        """
    ...
    def _time_imprecision_pm2(ts_: int, cited_feature_: str, cited_val_: int) -> Optional[int]:
        """
        Si cited_val_ correspond exactement à la MÊME feature à un ts' dans [ts-2, ts+2]
        (en excluant ts lui-même), retourne ts' matché, sinon None.
        """
    ...
    def _attribute_error_exclusive(ts_: int, cited_feature_: str, cited_val_: int) -> Dict[str, Any]:
        """
        Attribution EXCLUSIVE (pas de combinaison).
        Priorité: feature_confusion > time_imprecision > other
        """
        # 1) Confusion de feature au même ts (uniquement si ts existe)
        if ts_ in data_map:
        ...
        # 2) Imprécision temporelle ±2 (même feature)
    ...
    for feature, ts_str, val_str in matches:
        ...
        try:
            ...
            add_fail_example({
                ...
                "context": build_context_window(data_map, ts, window=context_window)
            })
            continue
        ...
        if ts not in data_map:
            ...
            msg = f"FAIL: {feature}@{ts} - Timestamp out of bounds"
            logs.append(msg)
            add_fail_example({
                "feature": feature,
                "raw_feature": raw_feature,
        ...
        if feature not in data_map[ts]:
            ...
            msg = f"FAIL: {feature}@{ts} - Feature not found (Available: {available})"
            logs.append(msg)
            add_fail_example({
                "feature": feature,
                "raw_feature": raw_feature,
        ...
        # 3. Check Value Accuracy (strict integer equality)
        ...
        if actual_val != val_cited:
            ...
            msg = f"FAIL: {feature}@{ts} - Cited {val_cited} vs Actual {actual_val}"
            logs.append(msg)
            add_fail_example({
                "feature": feature,
                "timestamp": ts,
    ...
    # Shares among false facts (exclusive categories)
...
def parse_context_data(prompt_text: str, plain_text: bool = False) -> Dict[int, Dict[str, int]]:
    ...
    IMPORTANT (mise à jour):
      - Toutes les valeurs sont des entiers, on parse en int strict.
    """
    ...
    def _to_int(raw: str) -> Optional[int]:
        raw = raw.strip()
        try:
            # Accepte "12" et aussi "12.0" si jamais ça traîne, mais refuse "12.3"
            if "." in raw:
                f = float(raw)
                if f.is_integer():
                    return int(f)
    ...
    if plain_text:
        ...
        # Captures key:value pairs like "timestamp: 12" or "moving_std: 5"
...
def print_summary(stats: Dict[str, Dict[str, int]], total_records: int):
    # Adjusted width to 85 to accommodate new column
    width = 85
    print("\n" + "="*width)
    ...
    for p_type in sorted(stats.keys()):
        ...
        # Calculate Percentage of Perfect Cases (False Count == 0)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate LLM citations in JSONL files.")
    parser.add_argument("input_file", help="Path to input .jsonl file")

    parser.add_argument("--prompt-key", default="input",
                        help="JSON key containing the data context (default: 'input')")
    parser.add_argument("--answer-key", default="input",
                        help="JSON key containing the text to evaluate (default: 'input')")

    parser.add_argument("--output", "-o", default="evaluated_output.jsonl",
    ...
    parser.add_argument("--max-records-with-fails", type=int, default=30,
                        help="Limite le nombre de records pour lesquels on imprime des erreurs (évite le spam).")
    parser.add_argument(
        "--plain-text",
        action="store_true",
    ...
    # Stats container: Added "perfect" to track cases with 0 false facts
    ...
    try:
        with open(args.input_file, 'r', encoding='utf-8') as fin, \
            ...
            for line in fin:
                ...
                # 1. Parse Data
                data_map = parse_context_data(prompt_text, plain_text=args.plain_text)
                ...
                if not data_map:
                    ...
                    eval_result = verify_citations(
                        ...
                        max_failed_examples=max(args.show_fails, 5)  # on garde au moins 5 en JSONL
                    )

                # Affichage console : quelques erreurs + contexte
                if args.show_fails > 0 and eval_result.get("false_count", 0) > 0:
                    # limiter le nombre de records imprimés
                    if "printed_fail_records" not in locals():
                    ...
                    if printed_fail_records < args.max_records_with_fails:
                        ...
                        print(f"false_count={eval_result.get('false_count')} | true_count={eval_result.get('true_count')} | score={eval_result.get('score')}")
                        ...
                        for i, ex in enumerate(failed, start=1):
                            ...
                            print(f"\n  [{i}] reason={ex.get('reason')}  feature={ex.get('feature')}  ts={ex.get('timestamp')}  attrib={cat}")
                            ...
                            if ex.get("reason") == "feature_missing":
                                print(f"      available_features_at_ts={ex.get('available_features_at_ts')}")
                            if cat == "feature_confusion_same_ts":
                                print(f"      matched_feature={attrib.get('matched_feature')}")
                            if cat == "time_imprecision_pm2":
                                print(f"      matched_timestamp={attrib.get('matched_timestamp')}")
                ...
                if "true_count" in eval_result:
                    ...
                    stats[p_type]["true"] += eval_result["true_count"]
                    stats[p_type]["false"] += eval_result["false_count"]
                    stats[p_type]["feature_confusion_errors"] += eval_result.get("feature_confusion_errors", 0)
                    stats[p_type]["time_imprecision_errors"] += eval_result.get("time_imprecision_errors", 0)
                    stats[p_type]["other_errors"] += eval_result.get("other_errors", 0)


                    # Check for "Clean" run (False Fact Counter == 0)
                    if eval_result["false_count"] == 0:
